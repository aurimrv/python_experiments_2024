...
# Script for generating the mutant report spreadsheet for Mutatest.
# Assumes that there is an RST report file generated by the tool.
...
def main():
    if len(sys.argv) < 3:
        ...
        print("Example: mutatestSummary.py /path files.txt test-sets.txt")
        sys.exit(1)
...
def processingMutatestMetrics(prj, clazz, mutatestDir, testSet, output):
    ...
    with open(reportFile, 'r') as f:
        ...
        for linha in f:
            ...
            matchError = re.search(r'- ERROR: (\d+)', linha) 
            if matchError:
                errorMutants = int(matchError.group(1))                   
        ...
        totalMutants = totalMutants - errorMutants
