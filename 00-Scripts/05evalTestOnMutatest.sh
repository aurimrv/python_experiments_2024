...
then
	...
	echo "Example: 05evalTestOnCosmicRay.sh /path files.txt test-sets.txt"
	exit
fi
...
do
	...
	do
		...
		/usr/bin/time -o ${tool}.time --quiet -p mutatest -s ${module}.py -t "python -m pytest --tb=no ./${tcDir}" -r 2023 -n=10000 -m f -o ${tcDir}/${tool}/${tcDir}-report.rst >& ${tool}.out
